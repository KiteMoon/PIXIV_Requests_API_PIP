...
def sql_sava(PID, NAME, AUTHOR, img_big_link_num, Width, Height, tags_dist, img_big_link_regular,
	...
	print("-----开始进行SQL-----")
	_data = (
	str(PID), NAME, AUTHOR, json.dumps(tags_dist, ensure_ascii=False), img_big_link_regular, img_big_link_original,
	img_big_link_num, Width, Height)
	# 全优化啦
	# print(_data) 需要的时候自行取消注释
	_sql = "INSERT INTO pixiv_day (PID,NAME,AUTHOR,TAGS,img_big_link,img_original_link,P_NUM,Width,Height) VALUES {}".format(
		_data)
	print("执行SQL语句"+_sql)
	try:
		cursor.execute(_sql)
